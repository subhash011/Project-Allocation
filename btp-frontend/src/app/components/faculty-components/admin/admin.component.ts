...
@Component({
    selector: "app-admin", templateUrl: "./admin.component.html", styleUrls: [ "./admin.component.scss" ], providers: [ LoginComponent ]
})
export class AdminComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        ...
        this.userService.getAdminInfo().subscribe((data) => {
            if (data["status"] == "success") {
                ...
                this.userService.getMembersForAdmin().subscribe((result) => {
                    dialogRefLoad.close();
    ...
    removeFaculty(id) {
        let dialogRef = this.dialog.open(DeletePopUpComponent, {
            height: "200px", width: "300px", data: {
                heading: "Confirm Deletion", message: "Are you sure you want to remove the Faculty?"
            }
        });
        dialogRef.afterClosed().subscribe((result) => {
            if (result && result["message"] == "submit") {
                const dialogRefLoad = this.dialog.open(LoaderComponent, {
                    data: "Removing, Please wait ...", disableClose: true, panelClass: "transparent"
                });
                this.userService.removeFacultyAdmin(id).subscribe((result) => {
                    dialogRefLoad.close();
                    if (result["message"] == "success") {
    ...
    removeStudent(student) {
        ...
        let dialogRef = this.dialog.open(DeletePopUpComponent, {
            height: "200px", width: "300px", data: {
                heading: "Confirm Deletion", message: "Are you sure you want to remove the Student?"
            }
        });
        dialogRef.afterClosed().subscribe((result) => {
            if (result && result["message"] == "submit") {
                ...
                this.userService.removeStudentAdmin(id).subscribe((result) => {
                    dialogRefLoad.close();
                    if (result["message"] == "success") {
                        this.studentCount--;
        ...
        if (date != null && date != "") {
            const dialogRef = this.dialog.open(DeletePopUpComponent, {
                width: "400px", height: "200px", data: {
                    heading: "Confirm Deadline", message: "Are you sure you want to fix the deadline?"
                }
            });
            dialogRef.afterClosed().subscribe((result) => {
    ...
    startAllocation() {
        ...
        const dialogRef = this.dialog.open(LoaderComponent, {
            data: "Allocating projects, Please wait as this may take a while", disableClose: true, panelClass: "transparent"
        });
        const length = this.students.data.filter((val) => val.isRegistered).length;
        this.userService.validateAllocation(selectedProjects, length).subscribe((data) => {
            if (data["status"] == "success") {
                this.projectService
                    ...
                    .subscribe((data) => {
                        ...
                        if (data["message"] == "success") {
                            ...
                            localStorage.setItem("allocationMap", JSON.stringify(data["allocationMap"]));
                            this.userService
                                .updatePublish("reset")
                                .subscribe((data) => {
                                    if (data["status"] == "success") {
    ...
    sendEmails() {
        const dialogRef = this.dialog.open(DeletePopUpComponent, {
            width: "400px", height: "200px", disableClose: false, hasBackdrop: true, data: {
                heading: "Confirm Sending Mails", message: "Are you sure you want to send the mails? This cannot be undone."
            }
        });
        dialogRef.afterClosed().subscribe((result) => {
            if (result && result["message"] == "submit") {
                const dialogRefLoad = this.dialog.open(LoaderComponent, {
                    data: "Sending mails, Please wait as this may take a while", disableClose: true, panelClass: "transparent"
                });
                if (this.stage_no == 0 || this.stage_no == 2) {
                    this.userService.getFacultyStreamEmails().subscribe((data1) => {
                        if (data1["status"] == "success") {
                            this.mailer
                                .adminToFaculty(this.stage_no, data1["result"], this.curr_deadline, data1["streamFull"])
                                .subscribe((data2) => {
                                    dialogRefLoad.close();
                    ...
                    this.userService.getStudentStreamEmails().subscribe((data1) => {
                        if (data1["status"] == "success") {
                            this.mailer
                                .adminToStudents(data1["result"], this.curr_deadline, data1["streamFull"])
                                .subscribe((data) => {
                                    dialogRefLoad.close();
                                    if (data["message"] == "success") {
                                        this.snackBar.open("Mails have been sent", "Ok");
    ...
    stepDownStage() {
        const dialogRef = this.dialog.open(DeletePopUpComponent, {
            width: "400px", height: "250px", data: {
                heading: "Confirm Proceed", message: "Are you sure you want to revert back to the previous stage?"
            }
        });
    ...
    resetProcess() {
        const dialogRef = this.dialog.open(ResetComponent, {
            width: "400px", height: "300px", disableClose: false, hasBackdrop: true
        });
        dialogRef.afterClosed().subscribe((result) => {
            if (result["message"] == "submit") {
                const dialogRefLoad = this.dialog.open(LoaderComponent, {
                    data: "Resetting, Please wait ...", disableClose: true, panelClass: "transparent"
                });
        ...
        if (this.fileToUpload.name.split(".")[1] == "csv") {
            const dialogRef = this.dialog.open(DeletePopUpComponent, {
                width: "450px", height: "200px", data: {
                    heading: "Confirm Upload", message: `Are you sure that you want to upload ${ this.fileToUpload.name } ?`
                }
            });
            dialogRef.afterClosed().subscribe((result) => {
                if (result && result["message"] == "submit") {
