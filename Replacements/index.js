...
var CountdownTimer = (function () {
	...
	CountdownTimer.prototype.getTimeDiff = function (datetime, useAsTimer) {
		...
		var /** @type {?} */ days = Math.floor(
				milisec_diff / 1000 / 60 / (60 * 24)
			);
		var /** @type {?} */ date_diff = new Date(milisec_diff);
		var /** @type {?} */ day_string = days
				? this.twoDigit(days) +
				  (this.twoDigit(days) == 1 ? " day, " : " days, ")
				: "";
		...
		if (this.timeOnly) {
			...
			return (
				this.twoDigit(hours) +
				(this.twoDigit(hours) == 1 ? " hour, " : " hours, ") +
				this.twoDigit(date_diff.getMinutes()) +
				(this.twoDigit(date_diff.getMinutes()) == 1 ? " minute " : " minutes ")
			); // +
			...
			return (
				...
				(this.twoDigit(date_diff.getUTCHours()) == 1 ? " hour, " : " hours, ") +
				this.twoDigit(date_diff.getMinutes()) +
				(this.twoDigit(date_diff.getMinutes()) == 1 ? " minute " : " minutes ")
			); // +
			// this.twoDigit(date_diff.getSeconds()) + (this.twoDigit(date_diff.getSeconds()) == 1 ? " second, " : " seconds");
